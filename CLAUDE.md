...
| `.github/workflows/daily-tweets.yml` | ~128 | GitHub Actions: 3 scheduled cron slots (13/17/21 UTC = 9am/1pm/5pm ET) driving `scripts/daily_tweets.py`. 13:00 = select + post slot 1 (`select-and-post-1`); 17:00 = post slot 2; 21:00 = post slot 3. Manual dispatch `mode` input adds `select-only` and `dry-run-all`. Dry-run by default — repo variable `TWEET_LIVE=1` flips to live. Commits updated `data/social-queue/` file each run, rebases against concurrent daily-scan commits before push. **Untracked as of 2026-06-04** (not yet committed). |
| `.github/workflows/daily-telegram.yml` | ~117 | GitHub Actions: mirrors the tweets workflow, offset +5 min (13:05/17:05/21:05 UTC) so the X workflow's queue commit has landed. `git pull --rebase` (with a 60s sleep-and-retry) before reading `data/social-queue/`. Runs `scripts/daily_telegram.py --slot N`. Dry-run by default — repo variable `TELEGRAM_LIVE=1` flips to live. Commits queue post-state back to `data/social-queue/`. **Untracked as of 2026-06-04** (not yet committed). |
...
- **One selection, two rails (X + Telegram)** — `daily_tweets.py --mode select` is the *only* Claude call; it writes the shared `data/social-queue/` file with per-channel content pre-rendered (incl. `telegram_caption_html`) and per-platform post-state. `daily_telegram.py` never selects — it just posts the already-chosen 3 picks. This keeps both platforms editorially aligned and halves Claude spend. The Telegram workflow runs +5 min after the X workflow each slot and rebases before reading, because the queue file is produced by the X run. Telegram delivery is free; X cost is driven by `TWEET_LINK_MODE`. As of 2026-06-04 the Telegram rail (`daily_telegram.py`, `daily-telegram.yml`, `TELEGRAM-SETUP.md`) and the migrated `daily-tweets.yml` are **untracked/uncommitted** in the working tree — verify they're pushed before assuming they run in CI.
