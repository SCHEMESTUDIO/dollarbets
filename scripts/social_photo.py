...
PER_QUERY = 8
RANK_POOL = 16
UA = "dollarbets-social/1.0 (+https://www.dollarbets.lol)"
...
def pexels(query, key):
    ...
    for ph in data.get("photos", []):
        out.append({"source": "pexels", "id": str(ph["id"]), "page": ph.get("url"),
                    "download": ph["src"]["large2x"], "photographer": ph.get("photographer", ""),
...
def openverse(query):
    ...
    for r in data.get("results", []):
        ...
        out.append({"source": "openverse", "id": r["id"], "page": r.get("foreign_landing_url"),
                    "download": r.get("url"), "photographer": creator,
                    "license": f"CC {lic} {ver}".strip(),
                    "credit": f"photo: {creator} · CC {lic}".strip()})
    return out
...
def choose_photo(market):
    ...
    def pool(source_fn, tag, loosen=False):
        ...
        for c in cands:
            try:
                c["img"] = _fetch_image(c["download"])
            except Exception as e:
                log(f"skip {c['source']}:{c['id']} ({e})")
                continue
